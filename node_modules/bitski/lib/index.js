...
const provider_shim_1 = require("./provider-shim");
...
const network_1 = require("./-private/network");
...
class Bitski {
    ...
    getProvider(options) {
        ...
        if (!provider) {
            this.provider = provider = new provider_shim_1.BitskiProviderShim(() => this.loadSDK(), typeof options === 'string' ? undefined : options);
        }
        if (network) {
    ...
    getConnectButton(options, callback) {
        return new connect_button_1.ConnectButton(() => this.loadSDK(), options, callback, () => this.getProvider());
    }
    /**
