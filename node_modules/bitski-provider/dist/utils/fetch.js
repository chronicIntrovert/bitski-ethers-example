...
export const fetchJsonWithRetry = async (fetchFn, retryCount, url, init, waitBeforeRetryFor = 0) => {
    try {
        ...
        if (!response.ok) {
            switch (response.status) {
                ...
                case 503:
                case 504:
                    throw ethErrors.rpc.internal('Gateway timeout. The request took too long to process. This can happen when querying logs over too wide a block range.');
                default:
                    // eslint-disable-next-line no-case-declarations
                    const text = await response.text();
                    // eslint-disable-next-line no-case-declarations
                    const errorMessage = maybeGetErrorMessage(text);
                    throw ethErrors.rpc.internal(errorMessage !== null && errorMessage !== void 0 ? errorMessage : `Request failed, response status: ${response.status}, body: ${text}`);
            }
        }
        ...
        if (err && isErrorRetriable(err)) {
            if (retryCount > 0) {
                ...
                throw ethErrors.rpc.internal(`FetchSubprovider - cannot complete request. All retries exhausted.\nOriginal Error:\n${err}\n\n`);
            }
        }
